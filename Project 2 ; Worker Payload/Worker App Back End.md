This is the tricky part since I learn a lot of new things

#### Part 0: Required Library
```python
import PySimpleGUI as sg
import io        # In this case to read the bytes information
import logging   # Detect/log event information
import locale    # Local Area Formatting
import ast       # Parse an expression (=[Operation])
import piexif    # Image rotation adjustments
import xlwings as xw
from PIL import Image
import create_worker_database as wb_com
import worker_payload_front_end as wb_front
```

#### Part 1: Database Insertion
```python
# Load Image: This wold be used everytime an image is shown on screen
def load_image(img_path):
	try:
		exif_dict = piexif.load(img_path)
		if '0th' in exif_dixt and piexif.ImageIFD.Orientation in exif_dict['0th']:
			...
			if orientation == 3:
				...
				image = Image.open(img_path).rotate(350, expand=True)
			else:
...
def put_input_to_database(values):
	try:
		...
		logging.error(f'Error while adding data to the database!')
		raise
...
#### Part 2: Getting UI Data
The most important take away is to use see the the name and date method since it's a new ons. 
```python
# Since there's 2 filter that should be updated, we would updated both values and window
# of both values and window
...
def get_cb_date(name, window):
	...
	window['-SELECT_WORKER_DATE-'].update(values=date)


def get_input_fill(window, values):
	name = values['-SELECT_WORKER_NAME-']
	date = values['-SELECT_WORKER_DATE-']
	# The backend method selects * from the name and date
...
def get_image_fill(window, values):
	...
	if retrieved_image is not None:
		...
		empty_image = r'D:\Projects\Workers Payload\default_picture.jfif'
		window['-IMAGE-'].update(data=load_img(empty_image))
		msg = 'You have not yet put an image\n Please consider putting one down!'
...
# This is also another important point since PySimpleGUI doesn't allow sg.Image 
# dynamic update. Therefor, a workaround would be creating a method to show the selected
# image.  
...
def edit_date(window, values):
	# Get The Date Using a Custom Popup Window
...
def edit_status(window, values):
	...
	# Method components (database key)
	name = values['-NAME-']
	date = values['-DATE-']
...
def edit_earning(values):
	...
	try:
		# Check, if no content/edit, then error is raised
        if new_earning is None:
            raise ValueError("No value is changed to a valid ones")
        # Changing new_earing data type
...
def clear_user_input(window):
	...
	window['-P_YES-'].update(values=False)
	window['-P_NO-'].update(value=False)
	window['-MLINE-'].update('')
```

#### Part 5: Data Deletion
```python
...
def instant_deletion(values):
	...
	try:
		if name is not None:
			# instant deletion with data containning name
			wb_com.instant_delete(name)
			sg.popup(f'{name} hase been removed', 'Removal Message')
	except Exception as e:
		sg.popup(f'Error: {e}')
```

#### Part 6: Program Operation 
```python
# Calculate total earn per filtered item
def calcualte_total_earn(window, value):
	name = values['-FILTER_WORKER_NAME-']
	month = values['-FILTER_WORKER_MONTH-']
	if name or month:
		filter_content = wb_com.get_table_filter(name, month)
...
# This would be the one used to translate the =[operation]
def evaluate_expression(expression):
	try:
		expression = expression.strip()
		# Detect Expression, if there's a = then this point onwards would be the operation
		if expression.startswith('='):
			expression = expression[1:]
		# Parse the expression
		node = ast.parse(expression, mode='eval')
		# Compile the parsed node
		complied_expr = compile(node, filename='<string>', mode='eval')
		# Evaluated and return the result
...
def calculate_total_presence(window, values):
	...
	if name or month:
		filter_content = wb_com.get_table_filter(name, month)
		total_attendance = sum(1 for row in filter content if row[2] == 1)
		total_presence = sum(1 for row in filter content if row[2] in [0,1])
...
#### Part 7: Export and View Excel
```python
def export_data(wb, filename):
	# Set the local environment/output to id
	locale.setlocale(locale.LC_ALL, 'id_ID')
	data = wb_com.get_export_data()
	# Open a new excel sheet
	sheet = wb.sheets[0]
	# Write the data to the worksheet columns
	# This would place the whole data upon the right left indexed item
	sheet.range('A1').value = data
	# This would format the date column
	...
	# Target and applies border to cells
	last_row = sheet.range('A' + str(sheet.cells.last_cell.row)).end('up').row
	last_column = sheet.range('A1').end('right').column
	sheet.range('A1', sheet.sheetcells(last_row, last_column)).api.Borders.LineStyle = 1
...
def export_file():
    ...
    if file_name:
		...
		sg.popup('Operation has been canceled')
...
def main_program():
    ...
    while True:
	    ...
	    if event == sg.WIN_CLOSED:
			...
			# Put the inputted data to the database
...
#### Part 9: Driver's Code
```python
if __name__ == '__main__':
    main_program()
```

#### Part 10: VS Code Source
```python 
# This one is the full rendition from vscode
import PySimpleGUI as sg
import io
import logging
import locale
